...
void calc_sources(int nx, int ny, double *x, double *y, double *dx, double *dy, double **T, double **b)
{
    ...
    for (i = 0; i < nx; i++)
        for (j = 0; j < ny; j++)
        {
            b[i][j] = -cos(M_PI*(j*dy[0]))*(2+M_PI*M_PI*i*dx[0]*(1-i*dx[0])); // set source function here
...
void get_coeffs(int nx, int ny, double *x, double *y, double *dx, double *dy, double **aP, double **aE, double **aW, double **aN, double **aS, double **b, double **T, double **kdiff, double **bcleft, double **bcrght, double **bctop, double **bcbot)
{
    ...
    for (i = 1; i < nx - 1; i++)
    {
        ...
        aP[i][j] = 2.0 * (1.0 + hxhy_sq) * pj - 2.0 * dy[0] * qj;
        aE[i][j] = -pj;
        aW[i][j] = -pj;
        aN[i][j] = -2.0 * hxhy_sq;
        aS[i][j] = 0.0;
        b[i][j] = b[i][j] * pj - 2.0 * dx[0] * dx[0] / dy[0] * rj;
    }
    ...
    for (i = 1; i < nx - 1; i++)
    {
        ...
        aP[i][j] = 2.0 * (1.0 + hxhy_sq) * pj + 2.0 * dx[0]* dx[0] / dy[0] * qj;
        aW[i][j] = -pj;
        aE[i][j] = -pj;
        aN[i][j] = 0.0;
        aS[i][j] = -2.0 * pj * hxhy_sq;
        b[i][j] = b[i][j] * pj + 2.0 * dx[0] * dx[0] / dy[0] * rj;
    }
