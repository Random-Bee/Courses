...
// edges - adjacency list, clk - vector clock
vector<int> edges;
vector<int> clk;
...
void doWork() {
    ...
    while(msgCnt < numMessages) {
        ...
        if(task < thr/(1+thr)) {
            ...
            inCnt++;
            // log the event
            logMsg[0] = 0; logMsg[1] = sizeof(clk); logMsg[2] = pid;
            logMsg[3] = inCnt; logMsg[4] = -1; logMsg[5] = t;
            for(int i=0; i<n; i++) {
                logMsg[i+6] = clk[i];
            }
            // send to p0 for logging
            MPI_Send(&logMsg, n+6, MPI_INT, 0, 0, MPI_COMM_WORLD);
        }
        // else, send a message to a random process
            ...
            // msg structure - process id, message count/id, vector clock => n+2 (in this order)
            int message[n+2];
            message[0] = pid; message[1] = msgCnt;
            logMsg[0] = 1; logMsg[1] = sizeof(clk)+sizeof(message); logMsg[2] = pid;
            logMsg[3] = msgCnt; logMsg[4] = dest; logMsg[5] = t;
            for(int i=0; i<n; i++) {
                message[i+2] = clk[i];
                logMsg[i+6] = clk[i];
            }
            // send the message to the destination
            MPI_Send(&message, n+2, MPI_INT, dest, 0, MPI_COMM_WORLD);
            // send to p0 for logging
            MPI_Send(&logMsg, n+6, MPI_INT, 0, 0, MPI_COMM_WORLD);
        }
...
void receiveMsgs() {
    while (1) {
        // log msg structure - event type, current size, pid, event id, dest, time, vector clock => n+6 (in this order)
        int message[n+2], logMsg[n+6];
        MPI_Recv(&message, n+2, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        // if message is -1, the message is a termination message, break the loop
        if(message[0]==-1) {
            break;
        }
        time_t t = time(0);
        logMsg[0] = 2; logMsg[1] = sizeof(clk)+sizeof(message); logMsg[2] = pid;
        logMsg[3] = message[1]; logMsg[4] = message[0]; logMsg[5] = time(0);
        // obtain the mutex and update vector clock
        sem_wait(&mtx);
        for(int i=0; i<n; i++) {
            clk[i] = max(clk[i], message[i+2]);
        }
        clk[pid-1]++;
        sem_post(&mtx);
        for(int i=0; i<n; i++) {
...
int main(int argc, char *argv[]) {
    ...
    if(pid==0) {
        ...
        ofstream file1;
        file1.open("log-VC.txt");
        int cnt = 0, size = 0;
        // run the loop until all processes have sent termination message
        while(cnt != n) {
            // log msg structure - event type, current size, pid, event id, dest, time, vector clock => n+6 (in this order)
            int message[n+6];
            MPI_Recv(&message, n+6, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            // internal event
            if(message[0]==0) {
        ...
        // fixed for normal VC
        file1 << "Average vector clock entries sent: " << n << endl;

        file1.close();

        cout << "Max space used(in bytes): " << size << endl;
        // fixed for normal VC
        cout << "Average vector clock entries sent: " << n << endl;
