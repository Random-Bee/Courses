...
// Class to store values of k, lambda1, lambda2 and thread id
class vals {
    public:
    int k;
    double lam1;
...
void testCS(vals val) {
    ...
    for(i=1;i<=val.k;i++) {
        ...
        int expected = 0, new_val = 1;
        while(!atomic_compare_exchange_strong(&lockCAS, &expected, new_val)){
        ...
        worst = max(worst,actEnterTime*1.0 - 1.0*reqEnterTime);
...
int main(int argc, char* argv[]) {
    ...
    // Open output file
    f_out = fopen("Output-CAS.txt","w");
    fprintf(f_out,"CAS ME Output\n");
