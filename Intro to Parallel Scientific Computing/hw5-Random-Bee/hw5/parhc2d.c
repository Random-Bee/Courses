...
void enforce_bcs(int nx, int ny, double *x, double *y, double **T, int istglob, int ienglob, int jstglob, int jenglob, int nxglob, int nyglob)
{
  ...
  // only enforce BCs on the global boundaries
...
void get_rhs(int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double dx, double dy, double *xleftghost, double *xrightghost, double *ybotghost, double *ytopghost, double kdiff, double *x, double *y, double **T, double **rhs)
{
  ...
  i = nx-1;
  if(ienglob == nxglob-1) {  // processors at the right end of the global domain
    for(j = 1; j < ny-1; j++)
      rhs[i][j] = 0.0;
  } else {
    for(j = 1; j < ny-1; j++)
  ...
  j = 0;
  if(jstglob == 0) {  // processors at the bottom end
    for(i = 1; i < nx-1; i++)
      rhs[i][j] = 0.0;
  } else {
    for(i = 1; i < nx-1; i++)
  ...
  j = ny-1;
  if(jenglob == nyglob-1) {  // processors at the top end
    for(i = 1; i < nx-1; i++)
      rhs[i][j] = 0.0;
  } else {
    for(i = 1; i < nx-1; i++)
...
void halo_exchange_2d_x(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *xleftghost, double *xrightghost, double *sendbuf_x, double *recvbuf_x)
{
  ...
  // Pack send buffer: send the leftmost column
  for(j = 0; j < ny; j++)
      sendbuf_x[j] = T[0][j];
  ...
  // Pack send buffer: send the rightmost column
  for(j = 0; j < ny; j++)
...
void halo_exchange_2d_y(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *ybotghost, double *ytopghost, double *sendbuf_y, double *recvbuf_y)
{
  ...
  // Pack send buffer: send the bottom row
  for(i = 0; i < nx; i++)
      sendbuf_y[i] = T[i][0];
  ...
  // Pack send buffer: send the top row
  for(i = 0; i < nx; i++)
      sendbuf_y[i] = T[i][ny-1];
  // Receive from bottom neighbor
  MPI_Recv(recvbuf_y, nx, MPI_DOUBLE, bot_nb, 1, MPI_COMM_WORLD, &status);
  MPI_Send(sendbuf_y, nx, MPI_DOUBLE, top_nb, 1, MPI_COMM_WORLD);
  for(i = 0; i < nx; i++)
...
void output_soln(int rank, int nx, int ny, int it, double tcurr, double *x, double *y, double **T, int px, int py)
{
  ...
  sprintf(fname, "T_x_y_%06d_%04d_%dx%d.dat", it, rank, px, py);
  //printf("\n%s\n", fname);
...
int main(int argc, char** argv)
{
  ...
  // ints    :: nxglob, nyglob, nx, ny num_time_steps, it_print, kdiff
  int *sendarr_int;
  sendarr_int = malloc(9*sizeof(int));
  if(rank==0)
  {
  ...
  MPI_Bcast(sendarr_int, 9, MPI_INT, 0, MPI_COMM_WORLD);
  if(rank!=0)
  {
  ...
  set_initial_condition(nx, ny, x, y, T, dx, dy, istglob, ienglob, jstglob, jenglob, nxglob, nyglob); // set initial condition
  ...
  printf("Time taken by rank %d per time step = %lf\n", rank, (end_time-start_time)/(double)num_time_steps);
