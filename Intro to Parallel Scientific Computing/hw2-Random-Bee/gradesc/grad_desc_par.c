...
double calc_Jfun(double *y, double *yhat, int m) {
    int i;
    double Jfun = 0.0;
    #pragma omp parallel for reduction(+:Jfun)
    for (i = 0; i < m; i++) {
...
void calc_new_theta(double **x, double *theta, double *yhat, double *y, double learning_rate, int m, int n) {
    ...
    for (j = 0; j < n + 1; j++) {
        gradJ = 0.0;
        #pragma omp parallel for reduction(+:gradJ)
        for (i = 0; i < m; i++) {
...
void scale_and_translate_data(double **x, double *y, double *means, double* sdevs, int m, int n)
{
  ...
  #pragma omp parallel
  {
    double local_means[n + 2];
    double local_sdevs[n + 2];
    ...
    for(i = 0; i < m; i++) {
      for(j = 0; j < n + 1; j++) {
        local_means[j] += x[i][j];
        local_sdevs[j] += x[i][j] * x[i][j];
      }
      local_means[n + 1] += y[i];
      local_sdevs[n + 1] += y[i] * y[i];
    }
    ...
    {
      for(j = 0; j < n + 2; j++) {
        means[j] += local_means[j];
        sdevs[j] += local_sdevs[j];
      }
    }
  }
...
{ 
  ...
  write_1darr_to_file("fit_parameters_par.dat", theta, n+1);
  write_data_to_file("fitting_results_scaled_par.dat", x, y, yhat, theta, m, n);

  undo_scale_and_translate_data(x, y, yhat, means, sdevs, m, n);
  //for(i=0;i<m;i++)
  //  printf("%lf, %lf, %lf, %lf, %lf, %lf\n", x[i][0], x[i][1], x[i][2], x[i][3], x[i][4], y[i]);
  write_data_to_file("fitting_results_unscaled_par.dat", x, y, yhat, theta, m, n);
