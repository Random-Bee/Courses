...
void solve_tridiagonal(int n, double a, double b, double c, double *rhs, double *sol)
{
    int i;
...
void linsolve_hc2d_gs_adi(int nx, int ny, double rx, double ry, double **rhs, double **T, double **Tnew)
{
    ...
    int i, j, k, max_iter = 100;
    double tol = 1.0e-6, norm_diff;
    ...
    for (k = 0; k < max_iter; k++) {
        ...
        for (j = 1; j < ny - 1; j++) {
            double *rhs_line = (double *)malloc(nx * sizeof(double));
            double *sol_line = (double *)malloc(nx * sizeof(double));
            for (i = 1; i < nx - 1; i++) {
                rhs_line[i] = rhs[i][j] + ry * (T[i][j - 1] + T[i][j + 1]) - 2*ry*T[i][j];
            }
            solve_tridiagonal(nx - 2, -rx, 1 + 2 * rx, -rx, &rhs_line[1], &sol_line[1]);
            for (i = 1; i < nx - 1; i++) {
                Tnew[i][j] = sol_line[i];
            }
            free(rhs_line);
        ...
        #pragma omp parallel for private(j)
        for (i = 1; i < nx - 1; i++) {
            double *rhs_line = (double *)malloc(ny * sizeof(double));
            double *sol_line = (double *)malloc(ny * sizeof(double));
            for (j = 1; j < ny - 1; j++) {
                rhs_line[j] = rhs[i][j] + rx * (Tnew[i - 1][j] + Tnew[i + 1][j]) - 2*rx*Tnew[i][j];
            }
            solve_tridiagonal(ny - 2, -ry, 1 + 2 * ry, -ry, &rhs_line[1], &sol_line[1]);
            for (j = 1; j < ny - 1; j++) {
                T[i][j] = sol_line[j];
            }
            free(rhs_line);
...
void output_soln(int nx, int ny, int it, double tcurr, double *x, double *y, double **T)
{
  ...
  sprintf(fname, "T_x_y_adi_%06d_%02d.dat", it, p);
  //printf("\n%s\n", fname);
