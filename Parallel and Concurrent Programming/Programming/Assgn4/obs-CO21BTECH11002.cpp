...
// Obstuction-free snapshot object
class OFS {
    // shared array
    ...
    OFS(int init) {
        ...
        for(int j=0; j<capacity; j++) {
            a_table[j].store(stampedValue(init, 0, 0));
        }
    }
    ...
    vector<int> scan() {
        ...
        oldCopy = collect();
        // keep scanning until two consecutive scans give same result
        while(true) {
            newCopy = collect();
            if(!equalStamps(oldCopy, newCopy)) {
                oldCopy = newCopy;
                continue;
            }
            // return the values
...
int main(int argc, char *argv[]) {
    ...
    // log file
    FILE *f_out = fopen("ofs-out.txt", "w");
